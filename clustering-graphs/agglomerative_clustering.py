...
centroids = []
data = normalized_data.loc[1:10, 't:0':'t:160'].reset_index(drop=True)
...
for i in range(len(data)):
    ...
    for j in range(len(data)):
        if i != j: 
            distance = sum([(a - b) ** 2 for a, b in zip(data.iloc[i], data.iloc[j])])
